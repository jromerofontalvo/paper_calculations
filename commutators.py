...
def ElectronCount(molecule):
  if molecule == 'H':
    n_electrons = 1
  elif molecule in ['H2', 'He', 'HeH+', 'H3+', 'LiH'] or molecule[:3] == 'H2_':
    n_electrons = 2
  elif molecule in ['Li', 'H3']:
    n_electrons = 3
  elif molecule in ['Be', 'H4']:
    n_electrons = 4
  elif molecule == 'B':
    n_electrons = 5
...
def GetHamiltonianTerms(molecule, integral_type,
                        add_conjugates=True, verbose=False, path=None):
  ...
  print 'commutators path:',path
  # Get the name of the file.
  if path!=None:
    name = '%s-%s.int' % (molecule, integral_type)
    name = path+name
    print 'commutators path:',path
    name=path
  else:
    name = 'from_jarrod/%s-%s.int' % (molecule, integral_type)
  ...
  with open(name, 'r') as file:
    for line in file:
      ...
      if counter==2:
        repulsionEnergy=float(data[0])

      if counter==8:
        FCIenergy=float(data[0])
        print "The FCIenergy from integrals file is:", FCIenergy
      ...
      # Two electron integral.
        ...
        # There really shouldn't be a negative sign here but there needs to be
        # for Jarrod's early files.
        coefficient = float(data[-1])
        integrals[term] = -1.0*coefficient
        n_orbitals = max(n_orbitals, max(term))

  # print "n_orbitals inside commutators:", n_orbitals
  ...
  return repulsionEnergy,coefficients, terms

# Function to load Jarrod's Hamiltonians.
def GetUCCDansatz(molecule, integral_type,
                        add_conjugates=True, verbose=False, path=None):
  """Parse and load molecular integral data for nonzero terms in normal form.
