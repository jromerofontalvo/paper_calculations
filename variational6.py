...
class Objective:
  ...
  def GetStatePrep(self, coefficients):
    ...
    for coefficient, term in zip(coefficients, self.terms):
      ...
      term_unit = GetUnitary(coefficient, term_matrix)
      # print 'Unitary'
      # print term_unit
...
def VariationallyMinimize(terms, reference_state, initial_guess,
  ...
  # solution = scipy.optimize.minimize(
      # ObjectiveFunction, initial_guess, method=method, options={'gtol':1e-06,'disp':True}).x
  method = 'BFGS'
  solution = scipy.optimize.minimize(
    ObjectiveFunction, initial_guess, method=method, options={'gtol':1e-06,'disp':True}).x
  # method = 'Powell'
  # solution = scipy.optimize.minimize(
  #   ObjectiveFunction, initial_guess, method=method, options={'ftol':0.000001,'xtol':0.0001}).x
  energy = ObjectiveFunction(solution)
  ncalls = ObjectiveFunction.calls
  return ncalls, solution, energy
...
def main():
  ...
  repulsion, coefficients, ham_terms = commutators.GetHamiltonianTerms(molecule, basis)
  ...
  if run_type=='troyer':
    ...
    for term in operators:
      # JRF
      hterm=MatrixForm(1., term, jw_terms, add_conjugates=True, anti_hermitian=False)
      hcontribution=Expectation(hterm, cisd_state)
  ...
  print 'Data %f %s %i %s.\n' % (thres,repr(float(variational_energy)),counter,repr(float(diff)))
